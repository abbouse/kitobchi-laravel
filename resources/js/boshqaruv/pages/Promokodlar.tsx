        ...
        <div className="table-responsive">
          <table className="data-table">
            ...
            <tbody>{promocodes.map((promo) => {
              ...
              return <tr key={promo.id}><td className="fw-semibold text-primary">#{promo.id}</td><td className="fw-bold" style={{ fontFamily: 'monospace', letterSpacing: 1 }}>{promo.code}</td><td className="fw-bold text-success">{isPercentPromo(promo.type) ? `${promo.discount}%` : `${fmt(promo.discount)} so'm`}</td><td><span className="chip chip-purple">{promoTypeLabel(promo.type)}</span></td><td>{promo.used} / {promo.max || '∞'}</td><td><div className="progress" style={{ width: 90, height: 6 }}><div className="progress-bar" style={{ width: `${percent}%`, background: percent > 80 ? '#ef4444' : '#10b981' }}></div></div></td><td className="text-muted">{promo.expiresAt || '—'}</td><td><span className={`chip ${promo.status === 'Active' ? 'chip-success' : 'chip-gray'}`}>{promo.status}</span></td><td><button className="btn btn-sm btn-light me-1" onClick={() => setSelected(promo)}><i className="bi bi-eye"></i></button><button className="btn btn-sm btn-light me-1" onClick={() => setEditing(promo)}><i className="bi bi-pencil"></i></button><button className="btn btn-sm btn-light text-danger" onClick={() => destroy(promo)}><i className="bi bi-trash"></i></button></td></tr>;
            })}</tbody>
          </table>
        </div>
      </div>
...
function PromoView({ promo, onHide, onEdit }: { promo: Promo | null; onHide: () => void; onEdit: () => void }) {
  return <Modal show={!!promo} onHide={onHide} centered><Modal.Header closeButton><Modal.Title className="fs-5 fw-bold">Promokod: {promo?.code}</Modal.Title></Modal.Header><Modal.Body><div className="row g-3"><Info label="Kod" value={promo?.code} mono /><Info label="Chegirma" value={isPercentPromo(promo?.type) ? `${promo?.discount}%` : `${fmt(promo?.discount || 0)} so'm`} /><Info label="Min. buyurtma" value={`${fmt(promo?.minOrder || 0)} so'm`} /><Info label="Max. chegirma" value={`${fmt(promo?.maxDiscount || 0)} so'm`} /><Info label="Bir user limiti" value={promo?.perUserLimit || 1} /><Info label="Ishlatilgan" value={`${promo?.used} / ${promo?.max || '∞'}`} /></div></Modal.Body><Modal.Footer><Button variant="outline-primary" onClick={onEdit}>Tahrirlash</Button><Button variant="light" onClick={onHide}>Yopish</Button></Modal.Footer></Modal>;
}
