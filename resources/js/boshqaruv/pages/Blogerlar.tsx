...
function BloggerDetail({ blogger, detail, onHide, onEdit, onShipment }: { blogger: Blogger | null; detail: Detail | null; onHide: () => void; onEdit: () => void; onShipment: (shipment?: Shipment) => void }) {
  ...
  return <Modal show={!!blogger} onHide={onHide} size="xl" centered><Modal.Header closeButton><Modal.Title className="fs-5 fw-bold">{blogger?.name}</Modal.Title></Modal.Header><Modal.Body>{!detail ? <div className="text-muted py-4 text-center">Ma'lumot yuklanmoqda...</div> : <div className="row g-3"><div className="col-lg-4"><div className="detail-panel h-100"><h6 className="fw-bold">Kontakt</h6><Info label="Telefon" value={detail.phone} /><Info label="Manzil" value={detail.address} /><Info label="Aktivlik" value={detail.activeUntil} /><Info label="Instagram" value={detail.instagramUrl} /><Info label="Telegram" value={detail.telegramUrl} /></div></div><div className="col-lg-8"><div className="detail-panel h-100"><div className="d-flex justify-content-between mb-3"><h6 className="fw-bold mb-0">Jo'natmalar</h6><button className="btn btn-sm btn-light" onClick={() => onShipment()}><i className="bi bi-plus-lg"></i></button></div>{detail.shipments.map((item) => <div className="border-bottom py-2" key={item.id}><div className="d-flex justify-content-between gap-2"><div><strong>{item.scheduledFor || '—'}</strong><div className="small text-muted">{item.status} · {item.note || ''}</div><div className="small">{(item.items || []).map((x) => x.name).join(', ')}</div></div><div><button className="btn btn-sm btn-light me-1" onClick={() => onShipment(item)}><i className="bi bi-pencil"></i></button><button className="btn btn-sm btn-light text-danger" onClick={() => removeShipment(item)}><i className="bi bi-trash"></i></button></div></div></div>)}{detail.shipments.length === 0 ? <div className="text-muted small">Jo'natma yo'q</div> : null}</div></div></div>}</Modal.Body><Modal.Footer>{detail ? <Button variant="outline-primary" onClick={onEdit}>Tahrirlash</Button> : null}<Button variant="light" onClick={onHide}>Yopish</Button></Modal.Footer></Modal>;
}
