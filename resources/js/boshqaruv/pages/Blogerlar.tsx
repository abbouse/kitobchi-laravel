...
export default function Blogerlar() {
  ...
  <div className="card-body"><div className="d-flex align-items-start gap-3 mb-3"><div className="d-flex-center b-r-50 bg-light-primary f-w-600 f-s-18 overflow-hidden flex-shrink-0 w-45 h-45">{initials(blogger.name)}</div><div className="min-w-0" style={{ flex: 1 }}><div className="f-w-600 text-truncate">{blogger.name}</div><small className="text-muted">{(blogger.platforms || []).join(', ') || blogger.phone || 'Bloger'} · {blogger.shipments || 0} shipment</small></div><span className={`badge ${blogger.status === 'Faol' ? 'text-light-success' : 'text-light-secondary'}`}>{blogger.status}</span></div><div className="f-s-13 text-muted mb-3">{blogger.address || blogger.activeUntil || '—'}</div><div className="d-flex gap-2"><button className="btn btn-sm btn-light-secondary flex-fill" onClick={() => open(blogger)}><i className="ti ti-eye"></i></button><button className="btn btn-sm btn-light-secondary flex-fill" onClick={() => setEditing(blogger)}><i className="ti ti-pencil"></i></button><button className="btn btn-light-danger icon-btn w-30 h-30 b-r-22" onClick={() => destroy(blogger)}><i className="ti ti-trash"></i></button></div></div>
  </div></div>)}</div>
