...
function ProfileModal({ user, detail, loading, onHide, reload }: { user: UserRow | null; detail: UserDetail | null; loading: boolean; onHide: () => void; reload: () => void }) {
  ...
  return (
    <Modal show={!!user} onHide={onHide} centered size="xl"><Modal.Header closeButton><Modal.Title className="f-s-20 f-w-600">{user?.name}</Modal.Title></Modal.Header><Modal.Body>
            {loading ? <div className="text-center py-5"><span className="spinner-border text-primary"></span><p className="text-secondary mt-2 mb-0">Profil yuklanmoqda...</p></div> : !detail ? <div className="alert alert-light-danger mb-0">Profilni yuklab bo‘lmadi.</div> : <div className="row">
              <div className="col-lg-4 col-xxl-3">
                <ProfileCard
                  ...
                  badges={<><Status user={user!} />{p.phoneVerified ? <span className="badge text-light-success">Telefon tasdiqlangan</span> : <span className="badge text-light-warning">Telefon tasdiqlanmagan</span>}{p.premium ? <span className="badge text-light-warning">Premium</span> : null}{p.support ? <span className="badge text-light-info">Support</span> : null}</>}
                  stats={[{ label: 'Buyurtma', value: fmt(Number(s.orders || 0)) }, { label: "To'langan", value: fmt(Number(s.paidOrders || 0)) }, { label: 'Kuzatuvchi', value: fmt(Number(s.followers || 0)) }]}
                />
