  ...
  return (
    <div>
      <div className="page-head">
        <div><h1 className="page-title">Foydalanuvchilar</h1><PageCrumbs /><p className="page-subtitle">Profil, online holat, kartalar, premium, bloklash va xarid tarixi boshqaruvi</p></div>
      </div>
      <div className="kpi-strip row g-3 mb-4">
        {[
          ['Jami', fmt(userCounts.all || 0), 'bi-people', 'var(--kc-ink)'], ['Online', fmt(userCounts.online || 0), 'bi-broadcast', 'var(--kc-ok)'],
      ...
      <div className="card-panel">
        ...
        <div className="table-responsive"><table className="table table-bottom-border align-middle data-table"><thead><tr><th>ID</th><th>Foydalanuvchi</th><th>Telefon</th><th className="right">Buyurtma</th><th className="right">Sarflangan</th><th className="right">Karta</th><th className="right">Oxirgi aktivlik</th><th>Holat</th><th></th></tr></thead><tbody>
          {users.map((user) => <tr key={user.id}>
            <td className="cell-id">#{user.id}</td><td><div className="d-flex align-items-center gap-2"><Avatar user={user} /><div><div className="fw-semibold">{user.name}</div><small className="text-muted">A'zo bo'lgan: {user.joinedLabel || user.joined || '—'}</small></div></div></td>
            <td className="money">{user.phone || '—'}</td><td className="right money">{user.orders}</td><td className="right money">{fmt(user.spent)}</td><td className="right money">{user.cards || 0}</td><td className="right">{user.online ? <span className="st ok"><i></i>Online</span> : <span className="cell-sub">{user.lastSeenAt || '—'}</span>}</td><td><Status user={user} /></td>
            <td><button className="btn btn-light-primary icon-btn w-30 h-30 b-r-22" onClick={() => openDetail(user)}><i className="bi bi-eye"></i></button></td>
          </tr>)}
...
function Status({ user }: { user: Pick<UserRow, 'status' | 'premium'> }) {
  ...
  return <span className={`st ${tone}`}><i></i>{label}</span>;
}
...
function ProductListBlock({ title, rows, empty, cart, alert, dateKey = 'addedAt', totalCount }: { title: string; rows: Row[]; empty: string; cart?: boolean; alert?: boolean; dateKey?: string; totalCount?: number }) {
  ...
  return <div className="col-12"><div className="detail-panel"><div className="d-flex align-items-center justify-content-between mb-3"><h6 className="fw-bold mb-0">{alert ? <><i className="bi bi-bell me-2 text-warning"></i>{title}</> : title}</h6><span className="money">{countLabel}</span></div><div className="row g-2">{rows.map((x, i) => { const outOfStock = Number(x.stock || 0) <= 0; return <div className="col-xl-6" key={String(x.id || i)}><div className="mini-stat h-100"><div className="d-flex gap-3 align-items-start"><div className="product-thumb-sm">{x.image ? <img src={String(x.image)} alt="" /> : <i className="bi bi-box-seam"></i>}</div><div className="min-w-0 flex-grow-1"><div className="d-flex flex-wrap gap-1 mb-1"><span className="chip chip-info">{String(x.typeLabel || x.productType || 'Mahsulot')}</span>{x.variant ? <span className="chip chip-purple">{String((x.variant as Row).name || 'Variant')}</span> : null}{alert ? <span className={`chip ${outOfStock ? 'chip-danger' : 'chip-success'}`}>{outOfStock ? 'Hozir tugagan' : 'Sotuvda bor'}</span> : <span className={`chip ${x.status === 'Faol' ? 'chip-success' : 'chip-gray'}`}>{String(x.status || '—')}</span>}</div><strong className="d-block text-truncate">{String(x.name || 'Mahsulot')}</strong><span className="d-block text-muted small">ID #{String(x.productId || '—')} · Stock: {fmt(Number(x.stock || 0))} · {dateLabel}: {String(x[dateKey] || '—')}</span><div className="d-flex flex-wrap gap-2 align-items-center mt-2"><span className="fw-semibold">{fmt(Number(x.unitPrice || x.price || 0))} so'm</span>{cart ? <span className="text-muted small">× {fmt(Number(x.quantity || 1))} = {fmt(Number(x.total || 0))} so'm</span> : null}{x.url ? <a className="btn btn-sm btn-light-secondary ms-auto" href={String(x.url)}><i className="bi bi-box-arrow-up-right me-1"></i>Product</a> : null}</div>{x.seller ? <span className="d-block text-muted small mt-1">Seller: {String(x.seller)}</span> : null}</div></div></div></div>; })}{rows.length === 0 ? <div className="col-12"><div className="text-muted text-center py-4">{empty}</div></div> : null}</div></div></div>;
}
function TableBlock({ title, rows, cols }: { title: string; rows: Row[]; cols: Array<[string, string]> }) {
  const hasLinks = rows.some((row) => row.url);
  return <div className="col-12"><div className="detail-panel"><h6 className="fw-bold mb-3">{title}</h6><div className="table-responsive"><table className="table table-bottom-border align-middle data-table mb-0"><thead><tr>{cols.map(([l]) => <th key={l}>{l}</th>)}{hasLinks ? <th>Amal</th> : null}</tr></thead><tbody>{rows.map((x, i) => <tr key={String(x.id || i)}>{cols.map(([, k]) => <td key={k}>{k === 'amount' ? `${fmt(Number(x[k] || 0))} so'm` : String(x[k] ?? '—')}</td>)}{hasLinks ? <td>{x.url ? <a className="btn btn-sm btn-light-secondary" href={String(x.url)}><i className="bi bi-box-arrow-up-right me-1"></i>Ochish</a> : '—'}</td> : null}</tr>)}{rows.length === 0 ? <tr><td colSpan={cols.length + (hasLinks ? 1 : 0)} className="text-center text-muted">Ma'lumot topilmadi</td></tr> : null}</tbody></table></div></div></div>;
}
...
function MapButtons({ mapLinks }: { mapLinks?: Record<string, string> }) {
  ...
  return <div className="d-flex gap-2 flex-wrap mt-2">{mapLinks.google ? <a className="btn btn-sm btn-light-secondary" href={mapLinks.google} target="_blank" rel="noreferrer"><i className="bi bi-geo-alt me-1"></i>Google Map</a> : null}{mapLinks.yandex ? <a className="btn btn-sm btn-light-secondary" href={mapLinks.yandex} target="_blank" rel="noreferrer"><i className="bi bi-map me-1"></i>Yandex Map</a> : null}</div>;
}
