  ...
  return (
    <div>
      <div className="d-flex align-items-end justify-content-between flex-wrap gap-3 mx-1 mb-3">
        <div><h4 className="main-title mb-0">Mystery Box</h4><PageCrumbs /><p className="mb-0 text-secondary">{mysteryBox.plans.length} ta plan · {mysteryBox.subscriptions.filter(s => s.status === 'active').length} ta faol obuna</p></div>
        <div className="d-flex gap-2"><button className="btn btn-primary" onClick={() => setPlanModal('create')}><i className="ti ti-plus me-1"></i>Yangi tarif</button></div>
      </div>
            ...
            <div className="card">
              <div className="card-body">
                <div className="d-flex justify-content-between align-items-center mb-2">
                  <div className="f-w-600 f-s-20">{p.name}</div>
                  <span className={`badge ${p.active ? 'text-light-success' : 'text-light-secondary'}`}>{p.active ? 'Faol' : 'Nofaol'}</span>
                </div>
                <div className="f-w-600 text-primary f-s-24">{p.price.toLocaleString()} so'm<small className="text-muted f-s-16">/oy</small></div>
                <div className="text-muted mb-2">{p.months} oy · {p.booksPerMonth} kitob/oy · {p.subscribers} obunachi</div>
                <div className="d-flex gap-2">
                  {p.updateUrl ? <button className="btn btn-light-success icon-btn w-30 h-30 b-r-22" onClick={() => setPlanModal(p)}><i className="ti ti-pencil"></i></button> : null}
                  {p.destroyUrl ? <button className="btn btn-light-danger icon-btn w-30 h-30 b-r-22" onClick={() => { if (confirm(`"${p.name}" tarifi o'chirilsinmi?`)) router.delete(p.destroyUrl!, { preserveScroll: true }); }}><i className="ti ti-trash"></i></button> : null}
                </div>
              </div>
            </div>
          </div>
...
<div className="card-body">
          ...
          <div className="table-responsive app-scroll"><table className="table table-bottom-border align-middle">
            ...
            <tbody>{mysteryBox.subscriptions.map(s => (
              <tr key={s.id}>
                <td className="f-w-600 text-primary">#{s.id}</td>
                <td className="f-w-600">{s.user}</td>
                <td><span className="badge text-light-primary f-s-9">{s.plan}</span></td>
                <td><span className={`badge ${s.status === 'active' ? 'text-light-success' : s.status === 'paused' ? 'text-light-warning' : 'text-light-secondary'} f-s-9`}>{s.statusLabel || s.status}</span></td>
                <td className="text-muted">{s.nextDelivery}</td>
                <td>
                  <button className="btn btn-light-primary icon-btn w-30 h-30 b-r-22 me-1" onClick={() => setSelected(s)}><i className="ti ti-eye"></i></button>
                  {s.status === 'active' ? <button className="btn btn-sm btn-warning me-1" onClick={() => patch(s.pauseUrl)}><i className="ti ti-player-pause-filled"></i></button> : null}
                  {s.status === 'paused' ? <button className="btn btn-sm btn-success me-1" onClick={() => patch(s.resumeUrl)}><i className="ti ti-player-play-filled"></i></button> : null}
                  {s.status === 'active' || s.status === 'paused' ? <button className="btn btn-light-danger icon-btn w-30 h-30 b-r-22" onClick={() => cancelSubscription(s.cancelUrl)}><i className="ti ti-x"></i></button> : null}
                </td>
              </tr>
            ))}</tbody>
          </table></div>
      ...
      <Modal show={!!planModal} onHide={() => setPlanModal(null)} centered>
        <Modal.Header closeButton><Modal.Title className="f-s-20 f-w-600">{planModal === 'create' ? "Yangi Mystery Box tarifi" : `"${(planModal as MysteryBoxPlan)?.name}" tarifini tahrirlash`}</Modal.Title></Modal.Header>
        {planModal ? (
          <MysteryBoxPlanForm
    ...
    <form onSubmit={(event) => submitLogistics(event, plan ? 'put' : 'post', action, onDone)}>
      <Modal.Body>
        <div className="row g-3">
          ...
          <div className="col-6">
            <label className="form-label f-s-13 text-muted f-w-600">Muddat (oy)</label>
          ...
          <div className="col-12">
            <label className="form-label f-s-13 text-muted f-w-600">Tavsif</label>
      ...
      <Modal.Footer>
        ...
        <button type="submit" className="btn btn-primary"><i className="ti ti-check me-1"></i>{plan ? 'Saqlash' : "Tarif qo'shish"}</button>
      </Modal.Footer>
    </form>
  );
}
...
export function SovgAlar() {
  ...
  return (
    <div>
      <div className="d-flex align-items-end justify-content-between flex-wrap gap-3 mx-1 mb-3">
        <div><h4 className="main-title mb-0">Sovg'alar</h4><PageCrumbs /><p className="mb-0 text-secondary">Jami {gifts.length} ta sovg'a mahsuloti</p></div>
      </div>
      <div className="card">
<div className="card-body">
          <div className="table-responsive app-scroll"><table className="table table-bottom-border align-middle">
            ...
            <tbody>{gifts.map(gift => (
              <tr key={gift.id}>
                <td><div className="w-40 h-55 b-r-10 overflow-hidden d-flex-center bg-light-primary flex-shrink-0">{gift.image ? <img className="w-100 h-100 object-fit-cover" src={gift.image} alt={gift.name} /> : <i className="ti ti-gift"></i>}</div></td>
                <td className="f-w-600">{gift.name}</td>
                <td>{gift.seller || '—'}</td>
                <td>{gift.priceFrom.toLocaleString()} - {gift.priceTo.toLocaleString()} so'm</td>
                <td>{gift.stock}</td>
                <td>{gift.sold}</td>
                <td><span className={`badge ${gift.approved ? 'text-light-success' : 'text-light-warning'}`}>{gift.approved ? gift.status : 'Moderatsiya'}</span></td>
                <td><span className="badge text-light-secondary">Ko'rildi</span></td>
              </tr>
            ))}</tbody>
          </table></div>
        </div>
...
function LogisticsToggle({ name, label, defaultChecked = false }: { name: string; label: string; defaultChecked?: boolean }) {
  return (
    <label className="d-flex align-items-center justify-content-between p-3 b-r-8 b-1-light h-100">
    ...
    <div className="b-r-15 b-1-light bg-light-subtle p-3">
      <div className="d-flex align-items-start justify-content-between gap-3 mb-3">
        <div>
          ...
          <div className="f-s-13 text-muted">Haqiqiy xaritadan markazni bosing, radiusni slider bilan belgilang. Toshkent uchun nom maydonlarini bo'sh qoldirish tavsiya qilinadi.</div>
        </div>
        <div className="d-flex align-items-center gap-2">
          <span className="badge text-light-success">{numericRadius.toFixed(numericRadius % 1 === 0 ? 0 : 1)} km</span>
          <div className="btn-group btn-group-sm">
            <button type="button" className="btn btn-light-secondary" onClick={() => setZoom((value) => Math.max(5, value - 1))}><i className="ti ti-minus"></i></button>
            <button type="button" className="btn btn-light-secondary disabled">{zoom}</button>
            <button type="button" className="btn btn-light-secondary" onClick={() => setZoom((value) => Math.min(15, value + 1))}><i className="ti ti-plus"></i></button>
          </div>
        </div>
      </div>
      ...
      <div
        ...
        tabIndex={0}
        onClick={handlePick}
        className="position-relative overflow-hidden b-r-15 b-1-light bg-light-secondary"
      ...
      >
        ...
        <div className="position-absolute b-r-50 bg-white b-1-light px-2 py-1 f-s-13 text-muted" style={{ top: 14, left: 14 }}>OpenStreetMap / CARTO</div>
        <div className="position-absolute" style={{ left: '50%', top: '50%', transform: 'translate(-50%, -50%)' }}>
          <div className="position-absolute b-r-50" style={{ width: radiusPx * 2,
            height: radiusPx * 2,
            left: -radiusPx,
            top: -radiusPx,
            background: 'color-mix(in srgb, rgba(var(--success), 1) 16%, transparent)',
            border: '1px solid color-mix(in srgb, rgba(var(--success), 1) 38%, transparent)',
          }} />
          <div className="position-relative d-flex align-items-center justify-content-center b-r-50 shadow text-white" style={{ width: 24, height: 24, background: 'rgba(var(--success), 1)', border: '3px solid rgba(var(--white), 1)' }}>
            <i className="ti ti-map-pin-filled f-s-11" />
          </div>
        </div>
        <div className="position-absolute f-s-13 text-muted bg-white bg-opacity-75 b-r-50 px-2 py-1" style={{ right: 10, bottom: 10 }}>
      ...
      <div className="mt-3">
        <label className="form-label f-s-13 text-muted f-w-600">Radius: {numericRadius.toFixed(numericRadius % 1 === 0 ? 0 : 1)} km</label>
    ...
    <form onSubmit={(event) => submitLogistics(event, service ? 'put' : 'post', action, onDone)}>
      ...
      <div className="row g-3">
        ...
        <div className="col-md-6">
          <label className="form-label f-s-13 text-muted f-w-600">Turi</label>
      ...
      <div className="text-end mt-4">
        <button className="btn btn-primary"><i className="ti ti-check me-1"></i>{service ? 'Saqlash' : "Xizmat qo'shish"}</button>
      </div>
    </form>
  );
}
...
function DeliveryRuleForm({ rule, services, action, onDone }: { rule?: DeliveryRule | null; services: DeliveryService[]; action?: string; onDone: () => void }) {
  ...
  const [color, setColor] = useState(rule?.color || (rule?.scope === 'radius' ? 'rgba(var(--success), 1)' : 'rgba(var(--primary), 1)'));
    ...
    <form onSubmit={(event) => submitLogistics(event, rule?.updateUrl ? 'put' : 'post', action, onDone)}>
      ...
      <div className="row">
        ...
        <div className="col-md-3">
          <label className="form-label f-s-13 text-muted f-w-600">Zona turi</label>
        ...
        <div className="col-md-2">
          <label className="form-label f-s-13 text-muted f-w-600">Rang</label>
        ...
        <div className="col-12">
          <div className="alert alert-border-secondary mb-0">
            <div className="f-w-600 mb-1">Nom maydonlari ixtiyoriy</div>
            <div className="f-s-13 text-muted">Toshkent kuryer zonalarida viloyat/tuman/shaharni bo'sh qoldiring. Shunda Yandex manzil nomlari ruscha yoki inglizcha kelsa ham geometriya (polygon/radius) bo'yicha to'g'ri ishlaydi.</div>
          </div>
        </div>
              ...
              <div className="card"><div className="card-body bg-light-subtle">
                  <div className="d-flex align-items-start justify-content-between gap-3 mb-2">
                    <div>
                      ...
                      <div className="f-s-13 text-muted">Kuryer yetkaza oladigan hududni xaritada erkin shakl qilib chizing.</div>
                    </div>
                    <span className={`badge ${polygon && polygon.length >= 3 ? 'text-light-primary' : 'text-light-secondary'}`}>{polygon ? polygon.length : 0} nuqta</span>
                  </div>
                  <YandexZoneEditor
                    scope="polygon"
                    lat={lat}
            ...
            <div className="col-12">
              <div className="card"><div className="card-body bg-light-subtle">
                  <div className="d-flex align-items-start justify-content-between gap-3 mb-2">
                    <div>
                      <div className="f-w-600">Radius xaritasi</div>
                      <div className="f-s-13 text-muted">Xaritaga bosing yoki markerni sudrang. Manzil qidiruvi ham ishlaydi.</div>
                    </div>
                    <span className="badge text-light-success">{radiusLabel} km</span>
                  </div>
                  <YandexZoneEditor
                    scope="radius"
                    lat={lat}
                  ...
                  <div className="mt-3">
                    <label className="form-label f-s-13 text-muted f-w-600">Radius: {radiusLabel} km</label>
        ...
        <div className="col-md-6">
          <label className="form-label f-s-13 text-muted f-w-600">Yetkazish xizmati</label>
      ...
      <div className="text-end mt-4">
        <button className="btn btn-primary"><i className="ti ti-check me-1"></i>{rule?.updateUrl ? 'Saqlash' : "Qoida qo'shish"}</button>
      </div>
    </form>
  );
}
...
export function Logistika() {
  ...
  const zoneColor = (rule: DeliveryRule) => rule.color || (rule.scope === 'polygon' ? 'rgba(var(--primary), 1)' : 'rgba(var(--success), 1)');
  ...
  return (
    <div>
      <div className="d-flex align-items-end justify-content-between flex-wrap gap-3 mx-1 mb-3">
        <div><h4 className="main-title mb-0">Logistika</h4><PageCrumbs /><p className="mb-0 text-secondary">Kuryer yetkazish zonalarini xaritada belgilang — polygon (taksi uslubi) yoki radius.</p></div>
        <div className="d-flex gap-2">
          <button className="btn btn-light-secondary" onClick={() => setEditingService(null)}><i className="ti ti-truck me-1"></i>Xizmat</button>
          <button className="btn btn-primary" onClick={() => setEditingRule(null)}><i className="ti ti-circle-plus me-1"></i>Zona qoidasi</button>
        </div>
      </div>

      <div className="row">
        {[
          { label: 'Xizmatlar', value: logisticsStats.services ?? deliveryServices.length, icon: 'ti-truck', color: 'rgba(var(--primary), 1)' },
          { label: 'Faol xizmat', value: logisticsStats.activeServices ?? deliveryServices.filter((item) => item.active).length, icon: 'ti-circle-check', color: 'rgba(var(--success), 1)' },
          { label: 'Polygon zona', value: logisticsStats.polygonRules ?? polygonRules.length, icon: 'ti-pentagon', color: 'rgba(var(--primary), 1)' },
          { label: 'Radius zona', value: logisticsStats.radiusRules ?? radiusRules.length, icon: 'ti-circle-dot', color: 'rgba(var(--success), 1)' },
          { label: 'COD qoidalari', value: logisticsStats.codRules ?? deliveryRules.filter((item) => item.codAllowed).length, icon: 'ti-coins', color: 'rgba(var(--primary), 1)' },
        ].map((item, kpiIndex) => (<div className="col-xl col-md-4 col-6" key={item.label}>
      ...
      <div className="row">
        {[
          { step: '1', title: 'Xizmat', icon: 'ti-truck', color: 'rgba(var(--primary), 1)', text: 'Kuryer yoki pochta kanali (Kitobchi kuryer, UzPost). Bir marta ochiladi.' },
          { step: '2', title: 'Zona chegarasi', icon: 'ti-pentagon', color: 'rgba(var(--primary), 1)', text: 'Polygon qilib xaritada chizasiz yoki radius berasiz — narx, COD, ETA shu yerda.' },
          { step: '3', title: 'Preview', icon: 'ti-calculator', color: 'rgba(var(--success), 1)', text: 'Koordinata kiritib, checkoutda qaysi yetkazish chiqishini oldindan tekshirasiz.' },
        ].map((item) => (
          <div className="col-md-4" key={item.step}>
            <div className="card h-100">
              <div className="card-body d-flex align-items-start gap-3">
                <div className="h-45 w-45 d-flex-center b-r-10 bg-light-primary f-s-20 flex-shrink-0"><i className={`${tiIcon(item.icon)}`}></i></div>
                <div>
                  <div className="f-w-600">{item.step}. {item.title}</div>
                  <div className="f-s-13 text-muted">{item.text}</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="row">
        <div className="col-xl-7">
          <div className="card h-100">
            <div className="card-header d-flex align-items-center justify-content-between gap-2 flex-wrap">
              <div>
                <h5 className="f-w-600">Zona qoidalari</h5>
                <p className="mb-0 text-secondary">A122 resolver ishlatadigan narx, radius, COD va priority sozlamalari</p>
              </div>
              <div className="d-flex gap-2 align-items-center">
                <button type="button" className="btn btn-sm btn-light-secondary" onClick={exportGeoJson} title="Zonalarni GeoJSON qilib yuklab olish"><i className="ti ti-download me-1"></i>GeoJSON</button>
                <form className="d-flex gap-2" onSubmit={(event) => submitLogistics(event, 'get', indexUrl)}>
                  ...
                  <button className="btn btn-light-secondary icon-btn w-30 h-30 b-r-22"><i className="ti ti-filter"></i></button>
                </form>
              </div>
            </div>
            <div className="card-body">
                ...
                <div className="nav nav-tabs app-tabs-primary flex-wrap">
                  {(['all', 'polygon', 'radius', 'country'] as const).map((value) => (
                    <div key={value} className="nav-item"><button
                        type="button"
                        className={`nav-link ${ruleScope === value ? 'active' : ''}`}
                        onClick={() => setRuleScope(value)}>
              ...
              <div className="table-responsive app-scroll">
                <table className="table table-bottom-border align-middle">
                  <thead><tr>
                    ...
                    <th>Scope</th>
                    <th>Xizmat</th>
                    <th className="cursor-pointer" style={{ userSelect: 'none' }} onClick={() => toggleSort('basePrice')}>Narx{sortCaret('basePrice')}</th>
                    <th className="cursor-pointer" style={{ userSelect: 'none' }} onClick={() => toggleSort('priority')}>Priority{sortCaret('priority')}</th>
                    <th>ETA</th>
                    <th>COD</th>
                    <th>Holat</th>
                    <th></th>
                  ...
                  <tbody>{sortedRules.map((rule) => (
                    <tr key={rule.id}>
                      <td>
                        <div className="f-w-600 d-flex align-items-center gap-2">
                          <span className="w-10 h-10 b-r-50 d-inline-block flex-shrink-0" style={{ background: zoneColor(rule) }}></span>
                          {rule.zoneName}
                        </div>
                        <p className="mb-0 text-secondary">{[rule.city, rule.district, rule.region, rule.country].filter(Boolean).join(', ') || '—'}</p>
                      </td>
                      <td>
                        <span className={`badge ${rule.scope === 'polygon' ? 'text-light-primary' : rule.scope === 'radius' ? 'text-light-success' : 'text-light-secondary'}`}>{rule.scope || '—'}</span>
                        {rule.scope === 'radius' ? <div className="f-s-13 text-muted">{rule.radiusKm || 0} km</div> : null}
                        {rule.scope === 'polygon' ? <div className="f-s-13 text-muted">{Array.isArray(rule.polygon) ? rule.polygon.length : 0} nuqta</div> : null}
                      </td>
                      <td>{rule.service || '—'}</td>
                      <td><strong>{(rule.basePrice || 0).toLocaleString()} so'm</strong><div className="f-s-13 text-muted">{rule.freePriceFrom ? `${rule.freePriceFrom.toLocaleString()} dan bepul` : 'chegara yoq'}</div></td>
                      <td className="f-w-600">{rule.priority ?? 0}</td>
                      <td>{rule.etaDays || 0} kun</td>
                      <td><span className={`badge ${rule.codAllowed ? 'text-light-success' : 'text-light-secondary'}`}>{rule.codAllowed ? 'Bor' : "Yo'q"}</span></td>
                      <td>
                        <button type="button" className="btn btn-sm p-0 border-0 bg-transparent" title={rule.active ? 'Nofaol qilish' : 'Faollashtirish'} onClick={() => toggleZoneActive(rule)}>
                          <i className={`ti ${rule.active ? 'ti-toggle-right text-success' : 'ti-toggle-left text-muted'} f-s-20`}></i>
                        </button>
                      </td>
                      <td className="text-end text-nowrap">
                        <button className="btn btn-light-secondary icon-btn w-30 h-30 b-r-22 me-1" title="Nusxalash" onClick={() => duplicateZone(rule)}><i className="ti ti-files"></i></button>
                        <button className="btn btn-light-success icon-btn w-30 h-30 b-r-22 me-1" title="Tahrirlash" onClick={() => setEditingRule(rule)}><i className="ti ti-pencil"></i></button>
                        <button className="btn btn-light-danger icon-btn w-30 h-30 b-r-22" title="O'chirish" onClick={() => removeLogistics(rule.destroyUrl, `${rule.zoneName} qoidasi o'chirilsinmi?`)}><i className="ti ti-trash"></i></button>
                      </td>
                    </tr>
                  ))}
                  {sortedRules.length === 0 ? <tr><td colSpan={9} className="text-center py-5 text-secondary"><i className="iconoir-archive d-flex justify-content-center mb-2 f-s-30 text-primary"></i>Mos zona qoidasi topilmadi</td></tr> : null}</tbody>
                </table>
              </div>
            </div>
          </div>
        ...
        <div className="col-xl-5">
          <div className="card h-100">
            <div className="card-header d-flex align-items-center justify-content-between gap-2 flex-wrap">
              <div>
                <h5 className="f-w-600"><i className="ti ti-pointer me-2 text-primary"></i>Aqlli preview</h5>
                <p className="mb-0 text-secondary">Xaritaga bosing — manzil qaysi zonaga tushishini darhol ko'rsatadi</p>
              </div>
            </div>
            <div className="card-body">
              ...
              {previewPoint ? (
                <div className="mt-3">
                  <div className="f-s-13 text-muted mb-2 d-flex align-items-center flex-wrap gap-2">
                    <span><i className="ti ti-map-pin me-1"></i>{previewPoint[0].toFixed(5)}, {previewPoint[1].toFixed(5)}</span>
                    {previewMatches.length > 1 ? <span className="badge text-light-warning">{previewMatches.length} zona mos · overlap</span> : null}
                    <button type="button" className="btn btn-light-danger icon-btn w-30 h-30 b-r-22 ms-auto" onClick={() => setPreviewPoint(null)}><i className="ti ti-x"></i></button>
                  </div>
                  {previewMatches.length === 0 ? (
                    <div className="alert alert-light-warning py-2 px-3 f-s-13 mb-0">Bu nuqta hech qaysi zonaga tushmaydi — checkoutda kuryer <b>ko'rsatilmaydi</b>.</div>
                  ) : (
                    <div className="d-flex flex-column gap-2">
                      {previewMatches.map((zone, index) => (
                        <div key={zone.id} className="p-2 b-r-8 b-1-light d-flex justify-content-between align-items-center gap-2" style={{ borderColor: index === 0 ? 'rgba(var(--danger-dark), 1)' : 'var(--border_color)', background: index === 0 ? 'rgba(var(--danger), .3)' : 'rgba(var(--white), 1)' }}>
                          <div>
                            <div className="f-w-600 d-flex align-items-center gap-2">
                              <span className="b-r-50 d-inline-block flex-shrink-0" style={{ width: 8, height: 8, background: index === 0 ? 'rgba(var(--danger-dark), 1)' : (zone.color || 'rgba(var(--secondary), 1)') }}></span>
                              {zone.zoneName}
                              {index === 0 ? <span className="badge text-light-danger">g'olib</span> : null}
                            </div>
                            <div className="f-s-13 text-muted">{zone.service || '—'} · {zone.scope} · priority {zone.priority}</div>
                          </div>
                          <div className="text-end">
                            <div className="f-w-600">{(zone.basePrice || 0).toLocaleString()} so'm</div>
                            <div className="f-s-13 text-muted">{zone.etaDays || 0} kun · COD {zone.codAllowed ? 'bor' : "yo'q"}</div>
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              <details className="mt-3">
                <summary className="f-s-13 text-muted cursor-pointer">Server orqali aniq narx (seller soni, summa bilan hisoblash)</summary>
                <form className="mt-2" onSubmit={(event) => submitLogistics(event, 'get', indexUrl)}>
                  ...
                  <div className="text-end mt-2"><button className="btn btn-light-secondary btn-sm"><i className="ti ti-calculator me-1"></i>Server narxi</button></div>
                </form>
                {logisticsPreview ? (
                  <div className="mt-2 d-flex flex-column gap-2">
                    {(logisticsPreview.offers || []).map((offer, index) => (
                      <div className="p-2 b-r-8 b-1-light" key={`${offer.service}-${index}`}>
                        <div className="d-flex justify-content-between gap-3"><strong>{offer.service || 'Xizmat'}</strong><span className="f-w-600">{(offer.price || 0).toLocaleString()} so'm</span></div>
                        <div className="f-s-13 text-muted">{offer.rule || 'qoida'} · {offer.etaDays || 0} kun · COD {offer.codAllowed ? 'bor' : "yo'q"}</div>
                      </div>
                    ))}
                    {(logisticsPreview.offers || []).length === 0 ? <div className="text-muted f-s-13">Mos yetkazish taklifi topilmadi.</div> : null}
                  </div>
                ) : null}
              </details>
            </div>
...
<div className="card-header d-flex align-items-center justify-content-between gap-2 flex-wrap">
          <div>
            <h5 className="f-w-600">Yetkazish xizmatlari</h5>
            <p className="mb-0 text-secondary">Kuryer va pochta xizmatlari endi Settings emas, Logistika ichida boshqariladi</p>
          </div>
          <div className="d-flex gap-2 align-items-center">
            <div className="position-relative w-200">
              <i className="ti ti-search position-absolute text-secondary" style={{ left: 12, top: 9 }}></i>
            ...
            <button className="btn btn-sm btn-light-secondary" onClick={() => setEditingService(null)}><i className="ti ti-circle-plus me-1"></i>Xizmat qo'shish</button>
          </div>
        </div>
<div className="card-body">
          ...
          <div className="table-responsive app-scroll">
            <table className="table table-bottom-border align-middle">
              ...
              <tbody>{filteredServices.map(service => (
                <tr key={service.id}>
                  <td className="f-w-600 text-primary">#{service.id}</td>
                  <td className="f-w-600">{service.name}</td>
                  <td><span className="badge text-light-secondary">{service.type || '—'}</span></td>
                  <td>{service.price === 0 ? <span className="badge text-light-success">Bepul</span> : <span className="f-w-600">{service.price.toLocaleString()} so'm</span>}</td>
                  <td>{service.freeFrom > 0 ? `${service.freeFrom.toLocaleString()} so'm` : '—'}</td>
                  <td className="f-w-600">{service.days} kun</td>
                  <td>{service.country || '—'} {service.capital ? '· poytaxt' : ''}</td>
                  <td><span className={`badge ${service.active ? 'text-light-success' : 'text-light-secondary'}`}>{service.active ? 'Faol' : 'Nofaol'}</span></td>
                  <td>
                    <button className="btn btn-light-success icon-btn w-30 h-30 b-r-22 me-1" onClick={() => setEditingService(service)}><i className="ti ti-pencil"></i></button>
                    <button className="btn btn-light-danger icon-btn w-30 h-30 b-r-22" onClick={() => removeLogistics(service.destroyUrl, `${service.name} xizmati o'chirilsinmi?`)}><i className="ti ti-trash"></i></button>
                  </td>
                </tr>
              ))}
              {filteredServices.length === 0 ? <tr><td colSpan={9} className="text-center py-5 text-secondary"><i className="iconoir-archive d-flex justify-content-center mb-2 f-s-30 text-primary"></i>Mos xizmat topilmadi</td></tr> : null}</tbody>
            </table>
          </div>
        </div>
</div>
      ...
      <Modal show={editingRule !== undefined} onHide={() => setEditingRule(undefined)} centered size="lg">
        <Modal.Header closeButton><Modal.Title className="f-s-20 f-w-600">{editingRule?.updateUrl ? 'Zona qoidasini tahrirlash' : 'Yangi zona qoidasi'}</Modal.Title></Modal.Header>
        <Modal.Body>
      ...
      <Modal show={editingService !== undefined} onHide={() => setEditingService(undefined)} centered size="lg">
        <Modal.Header closeButton><Modal.Title className="f-s-20 f-w-600">{editingService ? 'Xizmatni tahrirlash' : 'Yangi yetkazish xizmati'}</Modal.Title></Modal.Header>
        <Modal.Body>
