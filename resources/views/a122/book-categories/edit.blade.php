...
@php $isEdit = isset($bookCategory); @endphp
@section('title', $isEdit ? 'Kategoriya tahrirlash' : 'Yangi kategoriya')
@section('page-title', $isEdit ? 'Kategoriya tahrirlash' : 'Yangi kategoriya')

@section('content')
<div class="max-w-2xl">
  <div class="card p-6">
    <form method="POST" action="{{ $isEdit ? route('admin.book-categories.update', $bookCategory) : route('admin.book-categories.store') }}">
      @csrf
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input name="name_uz" value="{{ old('name_uz', $bookCategory->name_uz ?? '') }}" class="input" required>
        </div>
        <div>
          <label class="block text-xs font-semibold text-gray-500 mb-1">Nomi (Ruscha) <span class="text-rose-500">*</span></label>
          <input name="name_ru" value="{{ old('name_ru', $bookCategory->name_ru ?? '') }}" class="input" required>
        </div>
        <div>
          <label class="block text-xs font-semibold text-gray-500 mb-1">Nomi (Inglizcha)</label>
          <input name="name_en" value="{{ old('name_en', $bookCategory->name_en ?? '') }}" class="input">
        </div>
        <div>
          <label class="block text-xs font-semibold text-gray-500 mb-1">Nomi (Yaponcha)</label>
          <input name="name_ja" value="{{ old('name_ja', $bookCategory->name_ja ?? '') }}" class="input">
        </div>
        <div>
          <label class="block text-xs font-semibold text-gray-500 mb-1">Icon (emoji)</label>
          <input name="icon" value="{{ old('icon', $bookCategory->icon ?? '') }}" class="input" maxlength="10" placeholder="📚">
        </div>
      ...
      <div class="flex items-center gap-3 mt-6 pt-5 border-t border-gray-100 dark:border-white/5">
        ...
        <a href="{{ route('admin.book-categories.index') }}" class="btn btn-secondary">Bekor qilish</a>
      </div>
    </form>
  </div>
</div>
