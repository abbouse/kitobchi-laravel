        ...
        <tr class="legal-row">
          ...
          <td>
            <div class="legal-title">
              <div class="legal-title__text">{{ $policy->title }}</div>
              <div class="legal-title__meta">Legal page</div>
            </div>
          </td>
          <td>
            <span class="legal-slug">/legal/{{ $policy->slug }}</span>
          </td>
          <td>
            <div class="legal-preview">{{ strip_tags($policy->content) }}</div>
          </td>
...
<div class="mt-4">
  {{ $policies->links('a122.partials.pagination') }}
</div>
@endif

<div class="modal fade legal-modal" id="createModal" tabindex="-1">
  <div class="modal-dialog modal-dialog-scrollable">
    <form method="POST" action="{{ route('admin.policies.store') }}" id="createForm">
      @csrf
            ...
            <div class="legal-main">
              ...
              <section class="legal-panel">
                ...
                <div class="legal-panel__body">
                  ...
                  @include('a122.policies.partials.editor', ['id' => 'create'])
                  ...
                  <div class="legal-translations">
                    ...
                    <div class="legal-translation">
                      <div class="legal-translation__head">
                        <div class="legal-translation__title">{{ $lab }}</div>
                        <span class="badge badge-muted">{{ strtoupper($loc) }}</span>
                      </div>
                      <div class="legal-translation__body">
                        <div class="legal-field legal-field--surface mb-3">
                          <label class="p-form-label">Sarlavha</label>
                          <input type="text" name="translations[{{ $loc }}][title]" class="legal-input" maxlength="255" placeholder="Tarjima sarlavhasi">
                        </div>
                        <label class="p-form-label">Kontent</label>
                        @include('a122.policies.partials.editor', ['id' => 'create-'.$loc, 'content' => ''])
...
<div class="modal fade legal-modal" id="editModal-{{ $policy->id }}" tabindex="-1">
  <div class="modal-dialog modal-dialog-scrollable">
    <form method="POST" action="{{ route('admin.policies.update', $policy) }}" id="editForm-{{ $policy->id }}">
      @csrf @method('PUT')
      <div class="modal-content">
        <div class="modal-header">
          <div class="legal-modal__title">Tahrirlash: {{ $policy->title }}</div>
          <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body">
          <div class="legal-modal-grid">
            <div class="legal-main">
                ...
                <div class="legal-panel__body">
                  <div class="legal-fields">
                    <div class="legal-span-8">
                      <div class="legal-field legal-field--surface">
                        <label class="p-form-label">Sarlavha <span style="color:var(--p-danger)">*</span></label>
                        <input type="text" name="title" class="legal-input" value="{{ $policy->title }}" oninput="autoSlug(this,'edit-{{ $policy->id }}')" required>
                        <div class="legal-field__meta">Foydalanuvchi ko‘radigan asosiy hujjat nomi.</div>
                      </div>
                    </div>
                    <div class="legal-span-4">
                      <div class="legal-field legal-field--surface">
                        <label class="p-form-label">Slug</label>
                        <input type="text" name="slug" id="edit-{{ $policy->id }}-slug" class="legal-input slug-input" value="{{ $policy->slug }}">
                        <div class="legal-field__meta">URL ichida ishlatiladigan qisqa identifikator.</div>
                      </div>
                    </div>
                    <div class="legal-span-4">
                      <div class="legal-field legal-field--surface">
                        <label class="p-form-label">Tartib raqami</label>
                        <input type="number" name="sort_order" class="legal-input" value="{{ $policy->sort_order }}" min="0">
                        <div class="legal-field__meta">Ro‘yxatda chiqish tartibini belgilaydi.</div>
                      </div>
                    </div>
                    <div class="legal-span-4">
                      <div class="legal-toggle">
                        ...
                        <label class="legal-toggle__control">
                          <input type="hidden" name="show_in_app" value="0">
                          <input type="checkbox" name="show_in_app" value="1" {{ $policy->show_in_app ? 'checked' : '' }}>
                          Ilovada ko‘rsatilsin
                        </label>
                      </div>
                    </div>
                    <div class="legal-span-4">
                      <div class="legal-toggle">
                        ...
                        <label class="legal-toggle__control">
                          <input type="hidden" name="is_active" value="0">
                          <input type="checkbox" name="is_active" value="1" {{ $policy->is_active ? 'checked' : '' }} style="accent-color:var(--p-success)">
                          Faol holatda saqlash
                        </label>
                      </div>
                    </div>
              ...
              <section class="legal-panel">
                ...
                <div class="legal-panel__body">
                  <label class="p-form-label">Kontent (oʻzbekcha) <span style="color:var(--p-danger)">*</span></label>
                  @include('a122.policies.partials.editor', ['id' => 'edit-'.$policy->id, 'content' => $policy->content])
                  ...
                  <div class="legal-translations">
                    ...
                    <div class="legal-translation">
                      ...
                      <div class="legal-translation__body">
                        <div class="legal-field legal-field--surface mb-3">
                          <label class="p-form-label">Sarlavha</label>
                          <input type="text" name="translations[{{ $loc }}][title]" class="legal-input" maxlength="255" value="{{ $tr?->title ?? '' }}" placeholder="Tarjima sarlavhasi">
                        </div>
                        <label class="p-form-label">Kontent</label>
                        @include('a122.policies.partials.editor', ['id' => 'edit-'.$policy->id.'-'.$loc, 'content' => $tr?->content ?? ''])
