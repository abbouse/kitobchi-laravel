      ...
      @forelse($pendingPosts as $row)
        <div style="padding:16px 20px;border-bottom:1px solid var(--p-border)">
          <div class="flex flex-wrap items-start justify-between gap-3 mb-2">
            <div>
              <a href="{{ route('panel.book-club.show', $row) }}" class="font-semibold" style="color:var(--p-text);text-decoration:none">
                Post #{{ $row->id }}
              </a>
          ...
          @endif
          <form method="POST" action="{{ route('panel.book-club.post-ugc-score', $row) }}" class="flex flex-wrap items-end gap-2">
            @csrf
            ...
            <a href="{{ route('panel.book-club.show', $row) }}" class="btn-p ghost sm">Post sahifasi</a>
          </form>
        </div>
      @empty
      ...
      @forelse($pendingComments as $c)
        <div style="padding:16px 20px;border-bottom:1px solid var(--p-border)">
          <div class="flex flex-wrap items-start justify-between gap-2 mb-2">
            <div>
              @if($c->post)
                <a href="{{ route('panel.book-club.show', $c->post) }}" style="font-size:13px;font-weight:600;color:var(--p-accent);text-decoration:none">
                  Post #{{ $c->post_id }}
                </a>
              @else
          ...
          @endif
          <form method="POST" action="{{ route('panel.book-club.comment.ugc-score', $c) }}" class="flex flex-wrap items-end gap-2">
            @csrf
            <label class="text-xs" style="color:var(--p-hint)">Bahosi (1–5)</label>
            <select name="star" class="p-form-control" style="width:90px" required>
              @for($s = 1; $s <= 5; $s++)
            ...
            <button type="submit" class="btn-p primary sm"><i class="bi bi-check2"></i> Saqlash</button>
            @if($c->post)
              <a href="{{ route('panel.book-club.show', $c->post) }}" class="btn-p ghost sm">Post sahifasi</a>
            @endif
          </form>
        </div>
      @empty
