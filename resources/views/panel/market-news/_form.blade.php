        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div class="">
            ...
            <input type="text" name="title" class="p-form-control @error('title') border-danger @enderror"
                   value="{{ old('title', $marketNews?->title) }}"
          ...
          <div class="">
            ...
            <select name="align" class="p-form-control" required>
              <option value="top"    {{ old('align', $marketNews?->align) === 'top'    ? 'selected' : '' }}>
                ⬆ Yuqorida
              </option>
              <option value="center" {{ old('align', $marketNews?->align) === 'center' ? 'selected' : '' }}>
                ↔ O'rtada
              </option>
            </select>
          </div>
          ...
          <div class="">
            ...
            <label style="display:flex;align-items:center;gap:8px;cursor:pointer;margin-top:6px">
              ...
              <input type="checkbox" name="status" value="1"
                     {{ old('status', $marketNews?->status) ? 'checked' : '' }}
    ...
    <div class="p-card mb-3 fade-up">
      ...
      <div style="padding:0 18px 18px">
        ...
        <div class="row g-2 mb-3" id="actionBtns">
          ...
          ] as [$val, $label, $color, $icon, $hint])
          <div class="col-4">
            <input type="radio" name="action" id="act-{{ $val }}" value="{{ $val }}"
                   {{ old('action', $marketNews?->action ?? 'news') === $val ? 'checked' : '' }}
                   style="display:none">
            <label for="act-{{ $val }}" class="action-card"
              ...
              <i class="bi {{ $icon }}"
                 style="font-size:22px;color:var(--p-{{ $color }})"></i>
              <div style="font-size:12px;font-weight:500;color:var(--p-text)">{{ $label }}</div>
              <div style="font-size:10px;color:var(--p-hint);line-height:1.4">{{ $hint }}</div>
            </label>
          </div>
          @endforeach
        </div>
          ...
          <div class="flex gap-2 items-end">
            <div style="flex:1">
              ...
              <input type="number" name="action_id" id="actionIdInput"
                     class="p-form-control @error('action_id') border-danger @enderror"
                     value="{{ old('action_id', $marketNews?->action_id) }}"
      ...
      <div style="padding:0 18px 18px">
        ...
        <div id="dropZone"
          ...
          <div style="font-size:13px;color:var(--p-muted)">
            {{ $marketNews?->imgUrl ? 'Yangi rasm tanlash' : 'Rasm tanlang yoki tashlang' }}
          </div>
        ...
        @error('imgUrl')
          <div style="font-size:11px;color:var(--p-danger);margin-top:8px">{{ $message }}</div>
        @enderror
