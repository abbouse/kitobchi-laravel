  ...
  <div class="min-h-dvh bg-secondary-300 md:bg-gray-50 grow" v-if="product">
    ...
    <div class="md:hidden">
      ...
      <div class="bg-white p-4 sm:p-6 rounded-b-2xl">
        <div class="relative">
          ...
          <div role="region" aria-roledescription="carousel" data-orientation="horizontal" tabindex="0" data-slot="root" class="relative focus:outline-none w-full aspect-[3/4] rounded-xl overflow-hidden bg-white">
            <div data-slot="viewport" class="overflow-hidden w-full h-full">
              <div
                ref="mobileTrackEl"
          ...
          <div v-if="galleryImages.length > 1" class="absolute w-full bottom-0 right-0 p-2 z-20 flex justify-between items-end pointer-events-none">
          ...
          <div class="absolute top-4 right-4 z-30 flex items-center">
            <div class="relative overflow-hidden transition-shadow duration-300 rounded-full hover:shadow-sm hover:shadow-black/10 backdrop-blur-sm flex items-center gap-2 bg-white py-1.5 px-2.5 shadow-sm">
              <button
                type="button"
                @click.stop="favStore.toggleFavorite(product, 'book')"
              ...
              >
                <svg class="w-5 h-5 transition-colors duration-200" :class="isFav ? 'text-red-500 fill-red-500' : 'text-neutral-800'" viewBox="0 0 24 24" :fill="isFav ? 'currentColor' : 'none'" stroke="currentColor" stroke-width="1.5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.687-4.5-1.935 0-3.598 1.126-4.313 2.733-.715-1.607-2.377-2.733-4.312-2.733C5.098 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z"/>
                </svg>
              </button>
              <button
                type="button"
      ...
      <div class="relative z-20 bg-secondary-300 space-y-2 pb-44 pt-2">
        <!-- 1-Karta: Sarlavha, Do'kon nomi va Muallif -->
        <div class="relative z-20">
          <div class="px-4 sm:px-6 lg:px-8 w-full max-w-(--ui-container) mx-auto bg-white py-4 rounded-2xl">
            ...
            <div v-if="product.author" class="text-xs text-neutral-500 mt-1.5">
              Muallif: <span class="font-semibold text-neutral-800">{{ product.author }}</span>
            </div>
          </div>
        </div>
        ...
        <div v-if="hasSpecs" class="relative z-20">
          <div class="px-4 sm:px-6 lg:px-8 w-full max-w-(--ui-container) mx-auto bg-white py-4 rounded-2xl">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
              <div v-if="product.publisher" class="p-2.5 rounded-xl bg-secondary-50 flex justify-between items-center">
                <span class="text-neutral-500">Nashriyot:</span>
                <span class="font-semibold text-neutral-800">{{ product.publisher.name || product.publisher }}</span>
              </div>
              <div v-if="product.pages" class="p-2.5 rounded-xl bg-secondary-50 flex justify-between items-center">
                <span class="text-neutral-500">Sahifalar soni:</span>
                <span class="font-semibold text-neutral-800">{{ product.pages }} bet</span>
              </div>
              <div v-if="product.coverType" class="p-2.5 rounded-xl bg-secondary-50 flex justify-between items-center">
                <span class="text-neutral-500">Muqova:</span>
                <span class="font-semibold text-neutral-800">{{ product.coverType }}</span>
              </div>
              <div v-if="product.lang" class="p-2.5 rounded-xl bg-secondary-50 flex justify-between items-center">
                <span class="text-neutral-500">Til:</span>
                <span class="font-semibold text-neutral-800">{{ product.lang }}</span>
              </div>
              <div v-if="product.year" class="p-2.5 rounded-xl bg-secondary-50 flex justify-between items-center">
                <span class="text-neutral-500">Yil:</span>
                <span class="font-semibold text-neutral-800">{{ product.year }}</span>
              </div>
              <div v-if="product.isbn" class="p-2.5 rounded-xl bg-secondary-50 flex justify-between items-center">
              ...
              <div class="flex flex-col-reverse md:flex-row gap-3 h-full">
                ...
                >
                  <button
                    ...
                    :class="[
                      'relative shrink-0 w-[75px] h-[100px] rounded-xl overflow-hidden border-2 transition-all duration-300 cursor-pointer bg-secondary-50 p-0',
                  ...
                  <div role="region" aria-roledescription="carousel" class="relative focus:outline-none h-full w-full">
                    <div class="overflow-hidden w-full h-full rounded-2xl">
                      ...
                      >
                        ...
                        >
                          <div class="w-full h-[460px] lg:h-[500px] xl:h-[540px] rounded-2xl overflow-hidden bg-secondary-100 flex items-center justify-center relative">
                            <img
                              :src="img"
                              :alt="product.name"
            ...
            <div class="col-span-1 w-full">
              <div>
                ...
                <div class="w-full flex gap-2 mb-3">
                  <span class="font-medium inline-flex items-center text-sm px-2 py-1 bg-transparent border border-blue-500 gap-0.5 rounded-2xl text-blue-600">
                    <svg class="w-5 h-5 text-blue-600 fill-current" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.403 12.652a3 3 0 0 0 0-5.304a3 3 0 0 0-3.75-3.751a3 3 0 0 0-5.305 0a3 3 0 0 0-3.751 3.75a3 3 0 0 0 0 5.305a3 3 0 0 0 3.75 3.751a3 3 0 0 0 5.305 0a3 3 0 0 0 3.751-3.75Zm-2.546-4.46a.75.75 0 0 0-1.214-.883l-3.483 4.79l-1.88-1.88a.75.75 0 1 0-1.06 1.061l2.5 2.5a.75.75 0 0 0 1.137-.089l4-5.5Z" clip-rule="evenodd"/></svg>
                    Original
                  </span>
                </div>
                ...
                <div class="flex flex-col gap-6">
                  <div>
                    <h1 class="mb-3 text-2xl font-medium leading-tight text-neutral-900 m-0">{{ product.name }}</h1>
                    ...
                    <div v-if="product.author" class="text-sm font-medium text-neutral-500 mt-2">
                      Muallif: <span class="text-primary font-semibold">{{ product.author }}</span>
                    </div>
                    
                    <div class="flex flex-col mt-2">
                      <p class="text-sm text-neutral-400 font-normal m-0">Narxi</p>
                      <div class="flex items-center gap-1">
                        <div class="flex items-end gap-3">
                          <span class="text-xl font-bold text-neutral-900">{{ formatPrice(currentPrice) }} so'm</span>
                          <span v-if="discountPercent > 0" class="text-xl text-neutral-400 line-through">{{ formatPrice(product.price) }} so'm</span>
                        </div>
                        <span v-if="discountPercent > 0" class="px-2 py-1 bg-red-500 text-white text-xs font-bold rounded-lg">-{{ discountPercent }}%</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    <div
                      ...
                      class="w-full bg-secondary-300 cursor-pointer rounded-2xl p-4 md:px-6 flex items-center justify-between transition-colors duration-300 hover:bg-secondary-400"
                    >
                    ...
                    <div v-show="specsOpen" class="mt-3 p-5 rounded-2xl bg-secondary-50 space-y-3 border border-neutral-100/60">
                      <div v-if="hasSpecs" class="grid grid-cols-2 gap-3 text-sm">
                        <div v-if="product.publisher" class="p-3 rounded-xl bg-white">
                          <span class="text-neutral-400 block text-xs">Nashriyot:</span>
                          <span class="font-semibold text-neutral-800">{{ product.publisher.name || product.publisher }}</span>
                        </div>
                        <div v-if="product.pages" class="p-3 rounded-xl bg-white">
                          <span class="text-neutral-400 block text-xs">Sahifalar soni:</span>
                          <span class="font-semibold text-neutral-800">{{ product.pages }} bet</span>
                        </div>
                        <div v-if="product.coverType" class="p-3 rounded-xl bg-white">
                          <span class="text-neutral-400 block text-xs">Muqova:</span>
                          <span class="font-semibold text-neutral-800">{{ product.coverType }}</span>
                        </div>
                        <div v-if="product.lang" class="p-3 rounded-xl bg-white">
                          <span class="text-neutral-400 block text-xs">Til:</span>
                          <span class="font-semibold text-neutral-800">{{ product.lang }}</span>
                        </div>
                        <div v-if="product.year" class="p-3 rounded-xl bg-white">
                          <span class="text-neutral-400 block text-xs">Yil:</span>
                          <span class="font-semibold text-neutral-800">{{ product.year }}</span>
                        </div>
                        <div v-if="product.isbn" class="p-3 rounded-xl bg-white">
                          <span class="text-neutral-400 block text-xs">ISBN:</span>
                          <span class="font-semibold text-neutral-800">{{ product.isbn }}</span>
                        </div>
                      </div>
                    </div>
                  ...
                  <div class="p-6 rounded-3xl bg-secondary-100 space-y-5">
                    ...
                    <div v-if="paymentTab === 'installment'" class="flex max-md:flex-col md:justify-between gap-4 w-full">
                      <div>
                        <p class="text-sm text-neutral-500 font-normal m-0">Muddatli to'lov</p>
                        <div class="inline-flex mt-1 md:mt-2">
                          <div role="tablist" class="relative inline-flex bg-secondary-300 rounded-xl p-1 gap-1">
                            <button
                              v-for="m in installmentMonths"
                              :key="m"
                              type="button"
                              @click="selectedMonths = m"
                              :class="[
                                'px-3 py-1 text-xs rounded-lg font-semibold transition-all border-none cursor-pointer',
                      ...
                      <div class="flex flex-col md:items-end">
                        <p class="text-sm text-neutral-500 font-normal m-0">Muddatli to'lovga sotib olish</p>
                        <div class="flex items-end gap-1.5 mt-1 md:mt-4">
                          <span class="text-xl font-bold text-neutral-900">{{ formatPrice(monthlyForSelected) }}</span>
                          <span class="text-sm text-neutral-500">so'm/oyiga</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div v-else class="flex justify-between items-center w-full">
                      <div>
                        <p class="text-sm text-neutral-500 font-normal m-0">Narxi</p>
                    ...
                    <div class="flex items-center gap-2 sm:gap-3 gap-5">
                      <div class="flex-1">
                        <button
                          type="button"
                          @click="handleBuyNow"
                          class="ios-order-btn font-medium items-center transition-colors py-1.5 gap-1.5 text-white bg-primary hover:bg-primary/75 h-12 flex justify-center rounded-2xl text-base px-6 w-full flex-1 border-none cursor-pointer"
                        >
                          Buyurtma berish
                        </button>
                      </div>
                      <div>
                        <button
                          type="button"
                          @click="handleAddToCart"
                          class="font-medium inline-flex items-center text-base gap-2 text-primary bg-primary/10 hover:bg-primary/15 p-2 h-12 px-3.5 rounded-2xl border-none cursor-pointer"
                          aria-label="Savatga qo'shish"
                        >
...
const reviews = computed(() => {
  ...
  if (Array.isArray(preview) && preview.length > 0) {
    return preview.map((p: any) => ({
      id: p.id,
      user_name: [p.user?.name, p.user?.lastname].filter(Boolean).join(' ') || p.user?.name || 'Kitobxon',
...
async function openReviewsModal() {
  ...
  if (ugcList.value.length === 0 && rawId.value) {
    try {
      ...
      const res: any = await $fetch(`${config.public.apiBase}/v1/kitobchi/product_comments/${rawId.value}/book`)
