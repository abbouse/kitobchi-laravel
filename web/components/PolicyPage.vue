  ...
  <main class="max-md:pb-[71px] grow py-6 md:py-12 bg-[#F8FAFC]">
    <div class="px-4 sm:px-6 lg:px-8 w-full max-w-(--ui-container) mx-auto">
      <div class="max-w-3xl mx-auto">
        <!-- Breadcrumb -->
        <nav class="flex items-center gap-1.5 text-xs text-neutral-400 mb-6 md:mb-8 overflow-x-auto whitespace-nowrap py-1" aria-label="Breadcrumb">
          ...
          <NuxtLink to="/legal" class="hover:text-primary transition-colors">Huquqiy hujjatlar</NuxtLink>
          <svg class="w-3.5 h-3.5 shrink-0 text-neutral-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="m9 5 7 7-7 7" />
          </svg>
          <span class="text-neutral-700 font-medium truncate">{{ policy?.title || title }}</span>
        </nav>

        <div class="bg-white rounded-3xl p-6 sm:p-10 md:p-12 border border-neutral-200/60 shadow-xs">
          <!-- Yuklanmoqda -->
          <div v-if="pending" class="animate-pulse space-y-4">
            <div class="h-8 bg-neutral-100 rounded-lg w-2/3"></div>
            <div class="h-4 bg-neutral-100 rounded-lg w-1/3 mb-6"></div>
            <div class="h-4 bg-neutral-100 rounded-lg w-full"></div>
            <div class="h-4 bg-neutral-100 rounded-lg w-full"></div>
          ...
          <template v-else-if="policy">
            <h1 class="text-2xl sm:text-3xl md:text-4xl font-bold mb-3 text-neutral-900 tracking-tight leading-snug">{{ policy.title }}</h1>
            <p v-if="policy.updated_at" class="text-xs sm:text-sm text-neutral-400 mb-8 pb-6 border-b border-neutral-100">
              Oxirgi yangilanish: {{ formatDate(policy.updated_at) }}
            </p>
            <div class="kb-prose leading-relaxed text-neutral-700 space-y-4" v-html="policy.content"></div>
          </template>
        </div>
      </div>
    </div>
