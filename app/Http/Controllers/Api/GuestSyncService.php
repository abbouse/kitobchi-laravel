...
class GuestSyncController extends Controller
{
    // ─────────────────────────────────────────────────────────────────
    // CART SYNC
    // POST /cart_user/guest-sync
    // Body: {
    //   items: [
    //     { product_id, product_type, variant_id?, quantity }
    //   ]
    // }
    // ─────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────
    // FAVORITES SYNC
    // POST /user/favorites/guest-sync
    // Body: { items: [ { product_id, product_type } ] }
    // ─────────────────────────────────────────────────────────────────
    public function syncFavorites(Request $request)
    {
        ...
        foreach ($items as $raw) {
            try {
                ...
                if ($alreadyFaved) { $skipped++; continue; }

                // Mahsulot mavjudligini tekshirish (yashirilmagan)
    ...
    /**
     * Mahsulot mavjudligi va max stock ni qaytaradi.
     *
    ...
    private function checkProduct(
        ...
        if (!$stat) return [false, 0];

        // Variant bo'lsa — variant stock ni tekshiramiz
        if ($variantId) {
            $variant = $stat->variants()->where('id', $variantId)->first(['id', 'stock']);
            if (!$variant) return [false, 0];
    ...
    /**
     * User ning savatida shu mahsulot/variant bormi?
     */
