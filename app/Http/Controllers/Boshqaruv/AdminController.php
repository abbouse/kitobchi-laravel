...
PROMPT;
    ...
    private function dashboardPlatformAnalysis(): array
    {
        ...
        $platforms = [
            'android' => ['name' => 'Android', 'icon' => 'bi-android2', 'color' => 'var(--kc-ok)'],
            'ios' => ['name' => 'iOS', 'icon' => 'bi-apple', 'color' => 'var(--kc-text)'],
        ];
    ...
    {
        return [
            'inbound' => ['label' => 'Kelayotgan', 'icon' => 'bi-truck', 'color' => 'var(--kc-info)', 'statuses' => ['picked_from_seller', 'arrived_at_hub']],
            'qc' => ['label' => 'Nazorat (QC)', 'icon' => 'bi-clipboard-check', 'color' => 'var(--kc-cat-violet)', 'statuses' => ['qc_checked']],
            'packing' => ['label' => 'Qadoqlash', 'icon' => 'bi-box-seam', 'color' => 'var(--kc-cat-orange)', 'statuses' => ['packed']],
            'dispatch' => ['label' => 'Jo‘natish', 'icon' => 'bi-send', 'color' => 'var(--kc-cat-dgreen)', 'statuses' => ['labeled', 'dispatched_to_post', 'assigned_last_mile']],
            'delivery' => ['label' => 'Yetkazishda', 'icon' => 'bi-geo-alt', 'color' => 'var(--kc-cat-teal)', 'statuses' => ['out_for_delivery']],
        ];
    }
    ...
    {
        $colors = ['var(--kc-cat-indigo)', 'var(--kc-ok)', 'var(--kc-warn)', 'var(--kc-cat-plum)', 'var(--kc-cat-teal)', 'var(--kc-cat-violet)', 'var(--kc-danger)', 'var(--kc-cat-steel)'];
    ...
    {
        $colors = ['var(--kc-cat-plum)', 'var(--kc-cat-dviolet)', 'var(--kc-cat-navy)', 'var(--kc-ok)', 'var(--kc-warn)', 'var(--kc-cat-plum)', 'var(--kc-cat-teal)', 'var(--kc-cat-dred)'];
        $coords = [
    ...
    private function paymentSplit(array $paidStatuses, ?Carbon $start = null, ?Carbon $end = null): array
    {
        ...
        return [
            ['name' => 'To\'langan', 'count' => (int) $paid, 'share' => round($paid / $total * 100, 1), 'color' => 'var(--kc-ok)'],
            ['name' => 'Kutilmoqda', 'count' => $pending, 'share' => round($pending / $total * 100, 1), 'color' => 'var(--kc-warn)'],
            ['name' => "To'lov bekor qilingan", 'count' => $cancelled, 'share' => round($cancelled / $total * 100, 1), 'color' => 'var(--kc-danger)'],
        ];
    }
    ...
    private function dashboardConversionFunnel(?Carbon $start = null, ?Carbon $end = null): array
    {
        ...
        return Cache::remember('boshqaruv.dash.funnel.v1.'.$rangeKey, now()->addMinutes(2), function () use ($start, $end) {
            ...
            return [
                'stages' => [
                    ['key' => 'views', 'label' => "Ko'rishlar", 'value' => $views, 'rate' => 100.0, 'drop' => 0.0, 'color' => 'var(--kc-cat-dviolet)'],
                    ['key' => 'orders', 'label' => 'Buyurtma', 'value' => $orders, 'rate' => $rate($orders, $views), 'drop' => $views > 0 ? $rate($views - $orders, $views) : 0.0, 'color' => 'var(--kc-cat-violet)'],
                    ['key' => 'paid', 'label' => "To'langan", 'value' => $paid, 'rate' => $rate($paid, $orders), 'drop' => $orders > 0 ? $rate($orders - $paid, $orders) : 0.0, 'color' => 'var(--kc-cat-steel)'],
                    ['key' => 'completed', 'label' => 'Yakunlangan', 'value' => $completed, 'rate' => $rate($completed, $paid), 'drop' => $paid > 0 ? $rate($paid - $completed, $paid) : 0.0, 'color' => 'var(--kc-ok)'],
                ],
    ...
    private function fiscalizationPagePayload(): array
    {
        ...
        $pinfl = preg_replace('/\D+/', '', (string) ($cfg['pinfl'] ?? ''));
        if ($pinfl === '' && strlen($tin) === 14) {
            $pinfl = $tin;
            $tin = '';
        }
        $taxReady = strlen($pinfl) === 14 || strlen($tin) === 9;
        $taxLabel = strlen($pinfl) === 14 ? 'Platforma JSHSHIR (PINFL)' : 'Platforma STIR (TIN)';
        $taxValue = strlen($pinfl) === 14
            ? $this->maskSecret($pinfl)
            : (strlen($tin) === 9 ? $this->maskSecret($tin) : '9 xonali STIR yoki 14 xonali JSHSHIR kerak');
        $taxHint = strlen($pinfl) === 14 ? 'PAYLOV_OFD_PINFL' : 'PAYLOV_OFD_PINFL / PAYLOV_OFD_TIN';
        ...
        return [
            ...
            'fiscalHealth' => [
                ...
                ['key' => 'tin', 'label' => $taxLabel, 'ready' => $taxReady, 'value' => $taxValue, 'hint' => $taxHint],
