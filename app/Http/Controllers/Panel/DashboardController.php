    ...
    private function getData(): array
    {
        ...
        $recentOrders = Cache::remember('dash5_recent', $hot, fn () => Sold::with('user:id,name,lastname,avatar')->latest()->take(10)->get()->map(fn ($o) => [
            ...
            'status' => match ($o->status) {
                'C' => 'Yetkazildi','A' => 'Kutilmoqda','B' => "Yo'lda",'F' => 'Bekor qilindi',default => $o->status
            },
        ...
        if ($kangarooHumanReviewBooks > 0) {
            $alerts[] = ['warning', 'bi-book', 'Kangaroo: kitoblar (inson)',
                "{$kangarooHumanReviewBooks} ta kitob Kangaroo human_review — qo‘lda tasdiqlash", route('panel.books.index')];
        }
        if ($kangarooHumanReviewStationery > 0) {
            $alerts[] = ['warning', 'bi-pencil-square', 'Kangaroo: kanstovar (inson)',
                "{$kangarooHumanReviewStationery} ta mahsulot Kangaroo human_review — qo‘lda tasdiqlash", route('panel.stationery.index')];
        }
        if ($kangarooUgcAdminQueue > 0) {
            $alerts[] = ['warning', 'bi-stars', 'Kangaroo: UGC navbati',
                "{$kangarooUgcAdminQueue} ta post yoki izoh admin bahosini kutmoqda", route('panel.book-club.moderation-queue')];
        }
