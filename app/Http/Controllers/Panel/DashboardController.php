...
class DashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->has('clear_cache')) {
            foreach ($this->cacheKeys() as $key) Cache::forget($key);
            return redirect()->route('panel.dashboard')->with('success', 'Cache tozalandi.');
        }

        $admin        = auth('panel')->user();
        $isSuperAdmin = $admin?->role === 'superadmin';

        return view('panel.dashboard', array_merge(
            $this->getData(),
    ...
    private function cacheKeys(): array
    {
        return [
            ...
            'dash5_daily_ord','dash5_monthly','dash5_monthly_fin',
    ...
    private function getData(): array
    {
        ...
        $isolatedUsers = Cache::remember('dash5_u_isolated', $ttl, fn() =>
            User::whereNull('verifyCode')
        ...
        $dailyNewUsers = Cache::remember('dash5_u_daily', $ttl, fn() =>
        ...
        $dailyOrders = Cache::remember('dash5_daily_ord', $ttl, fn() =>
            collect(range(6,0))->map(fn($i) => [
                'day'   => now()->subDays($i)->format('D'),
        ...
        $monthlyRevenue = Cache::remember('dash5_monthly', $ttl, fn() =>
        ...
        $monthlyFinancial = Cache::remember('dash5_monthly_fin', $ttl, fn() =>
            collect(range(5,0))->map(function ($i) {
                $m = now()->subMonths($i); $mo = $m->month; $yr = $m->year;
        ...
        try {
            ...
            $repeatBuyersMonth = Cache::remember('dash5_repeat', $ttl, fn() =>
                DB::table('solds')->select('user_id')->where('paymentStatus',2)
                    ->whereMonth('created_at',now()->month)->groupBy('user_id')
                    ->havingRaw('COUNT(*) > 1')->get()->count()
            );
            $newBuyersMonth = Cache::remember('dash5_new_buyers', $ttl, fn() =>
                DB::table('solds')->select('user_id')->where('paymentStatus',2)
                    ->whereMonth('created_at',now()->month)->groupBy('user_id')
                    ->havingRaw('COUNT(*) = 1')->get()->count()
            );
            ...
            $aovMonthly = Cache::remember('dash5_aov_monthly', $ttl, fn() =>
                collect(range(5,0))->map(function ($i) {
                    $m = now()->subMonths($i);
            ...
            $deliveryTypeSplit = Cache::remember('dash5_delivery_split', $ttl, fn() =>
                Sold::where('paymentStatus',2)->whereNotNull('deliveryType')
        ...
        $activeCouriers  = Cache::remember('dash5_c_active',   $ttl, fn() => Couriers::where('status',1)->count());
        ...
        try {
            ...
            $giftSent         = GiftCertificate::where('status','sent')->count();
            $giftRevenue      = (int) GiftCertificate::where('status','used')->sum('nominal_uzs');
            $giftExpiringSoon = GiftCertificate::where('status','sent')
        ...
        $topMixedProducts = Cache::remember('dash5_top_mixed', $ttl, function () {
            ...
            return collect($top)->map(function ($s) use ($books,$stats2) {
                ...
                $model->_type=$s['type']; $model->sold_count=$s['sold_count']; $model->total_revenue=(int)$s['total_revenue'];
                return $model;
            })->filter()->values();
        });
        ...
        // ── RECENT ORDERS ─────────────────────────────────────
        $recentOrders = Cache::remember('dash5_recent', $hot, fn() =>
            Sold::with('user:id,name,lastname,avatar')->latest()->take(10)->get()->map(fn($o) => [
                'id'       => $o->id,
                'customer' => $o->user ? trim($o->user->name.' '.$o->user->lastname) : 'Mehmon',
                'avatar'   => $o->user?->avatar,
                'amount'   => number_format($o->amount),
                'status'   => match($o->status){'C'=>'Yetkazildi','A'=>'Kutilmoqda','B'=>"Yo'lda",'F'=>'Bekor qilindi',default=>$o->status},
        ...
        if ($pendingOrders > 0)
            $alerts[] = ['danger','bi-bag-x','Kutilayotgan buyurtmalar',
                "{$pendingOrders} ta buyurtma qabul qilinmagan",route('panel.orders.index',['status'=>'A'])];

        if ($pendingSellers > 0)
            $alerts[] = ['warning','bi-shop-window','Yangi sotuvchi arizasi',
                "{$pendingSellers} ta sotuvchi tasdiqlash kutmoqda",route('panel.sellers.index',['status'=>'pending'])];

        if ($pendingSellerTxCount > 0)
            $alerts[] = ['warning','bi-arrow-left-right','Seller to\'lov arizasi',
                "{$pendingSellerTxCount} ta — ".number_format($pendingSellerPayout)." UZS",route('panel.seller-transactions.index',['tab'=>'pending'])];

        if ($pendingCourierTxCount > 0)
            $alerts[] = ['warning','bi-bicycle','Kuryer to\'lov arizasi',
                "{$pendingCourierTxCount} ta — ".number_format($pendingCourierPayout)." UZS",route('panel.seller-transactions.index',['tab'=>'pending','segment'=>'courier'])];

        if ($mysteryDueCount > 0)
            $alerts[] = ['danger','bi-box-seam','Mystery Box navbati',
                "{$mysteryDueCount} ta obunachi jo'natish kutmoqda",route('panel.mystery-box.subscriptions',['tab'=>'active'])];

        if ($giftExpiringSoon > 0)
            $alerts[] = ['warning','bi-gift','Sertifikat muddati tugaydi',
                "{$giftExpiringSoon} ta sertifikat 7 kun ichida bekor bo'ladi",route('panel.gift-certificates.index',['tab'=>'sent'])];

        if ($stalePreparing > 0)
            $alerts[] = ['warning','bi-hourglass-split','Mystery Box kechikdi',
                "{$stalePreparing} ta yetkazish 2+ kun tayyorlanmagan",route('panel.mystery-box.subscriptions')];

        try {
            $pr = Report::where('status','pending')->count();
            if ($pr > 0) $alerts[] = ['danger','bi-flag-fill','Ko\'rib chiqilmagan shikoyat',
                "{$pr} ta shikoyat javob kutmoqda",route('panel.reports.index')];
        } catch (\Throwable) {}

        try {
            $qt = BotTicket::where('status','queue')->count();
            if ($qt > 0) $alerts[] = ['warning','bi-headset','Support navbati',
                "{$qt} ta murojaat javob kutmoqda",route('panel.bot-tickets.index')];
        } catch (\Throwable) {}

        try {
            $pa = SellerAd::where('moderation','pending')->count();
            if ($pa > 0) $alerts[] = ['warning','bi-megaphone','Reklama moderatsiya',
                "{$pa} ta reklama tasdiqlash kutmoqda",route('panel.seller-ads.index')];
        } catch (\Throwable) {}
        ...
        return compact(
            ...
            'completedOrders','pendingOrders','onwayOrders','cancelledOrders',
            'completionRate','cancellationRate','dailyOrders','monthlyRevenue',
