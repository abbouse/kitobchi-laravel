...
class SettingsController extends Controller
{
    ...
    public function updateVersions(Request $request)
    {
        ...
        ProjectSettings::first()->update($request->only([
    ...
    public function storeCommission(Request $request)
    {
        ...
        CommissionSettings::create($request->only(['priceFrom', 'priceTo', 'percent']));
        return back()->with('success', 'Komissiya qoidi qo\'shildi.');
    }

    public function updateCommission(Request $request, CommissionSettings $commissionSettings)
    {
        $request->validate([
            'priceFrom' => 'required|integer|min:0',
            'priceTo'   => 'required|integer|gt:priceFrom',
        ...
        $commissionSettings->update($request->only(['priceFrom', 'priceTo', 'percent']));
        return back()->with('success', 'Komissiya yangilandi.');
    }

    public function destroyCommission(CommissionSettings $commissionSettings)
    {
    ...
    public function storeCashback(Request $request)
    {
        ...
        CashbackSettings::create($request->only(['fromUzs', 'toUzs', 'cashback']));
        return back()->with('success', 'Cashback qoidasi qo\'shildi.');
    }

    public function updateCashback(Request $request, CashbackSettings $cashbackSettings)
    {
        $request->validate([
            'fromUzs'  => 'required|integer|min:0',
            'toUzs'    => 'required|integer|gt:fromUzs',
        ...
        $cashbackSettings->update($request->only(['fromUzs', 'toUzs', 'cashback']));
        return back()->with('success', 'Cashback yangilandi.');
    }

    public function destroyCashback(CashbackSettings $cashbackSettings)
    {
        $cashbackSettings->delete();
