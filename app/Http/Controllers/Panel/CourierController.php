...
class CourierController extends Controller
{
    ...
    {
        $query = Couriers::withCount('devices');
        ...
        $regions = Couriers::distinct()->pluck('region')->filter()->sort()->values();

        return view('panel.couriers.index', compact('couriers', 'counts', 'tab', 'regions'));
    }

    public function create()
    {
        return view('panel.couriers.create');
    }
    ...
    public function show(Couriers $courier)
    {
        ...
        $pendingPay  = CourierTransaction::where('courier_id', $courier->id)
            ->where('status', 'pending')->sum('amount');

        $recentOrders = CourierOrder::where('courier_id', $courier->id)
        ...
        $devices = $courier->devices()->latest()->take(3)->get();

        return view('panel.couriers.show', compact(
            'courier',
            'orderCount', 'totalEarned', 'pendingPay',
            'recentOrders', 'transactions', 'banLogs', 'devices'
        ));
    }

    public function edit(Couriers $courier)
    {
        return view('panel.couriers.edit', compact('courier'));
    }
    ...
    public function update(Request $request, Couriers $courier)
    {
        $data = $request->validate([
            ...
            'phone_number' => ['required', 'string',
                Rule::unique('couriers', 'phone_number')->ignore($courier->id)],
            'region'       => 'required|string|max:50',
        ...
        return redirect()->route('panel.couriers.show', $courier)
            ->with('success', "Kuryer yangilandi.");
    }
    ...
    public function destroy(Couriers $courier)
    {
        if ($courier->photo) Storage::disk('public')->delete($courier->photo);
        $courier->delete();
        return redirect()->route('panel.couriers.index')
            ->with('success', "Kuryer o'chirildi.");
    }

    public function export(Request $request)
    {
        return Excel::download(
            new CouriersExport($request->all()),
            'couriers_' . now()->format('Y-m-d') . '.xlsx'
        );
    }
}