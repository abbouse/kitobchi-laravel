<?php $isEdit = isset($stationeryCategory); ?>
<?php $__env->startSection('title', $isEdit ? 'Kategoriya tahrirlash' : 'Yangi kategoriya'); ?>
<?php $__env->startSection('page-title', $isEdit ? 'Kategoriya tahrirlash' : 'Yangi kanstovar kategoriyasi'); ?>
    ...
    <form method="POST" action="<?php echo e($isEdit ? route('admin.stationery-categories.update', $stationeryCategory) : route('admin.stationery-categories.store')); ?>">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input name="name_uz" value="<?php echo e(old('name_uz', $stationeryCategory->name_uz ?? '')); ?>" class="input" required>
        </div>
        <div>
          <label class="block text-xs font-semibold text-gray-500 mb-1">Nomi (Ruscha) <span class="text-rose-500">*</span></label>
          <input name="name_ru" value="<?php echo e(old('name_ru', $stationeryCategory->name_ru ?? '')); ?>" class="input" required>
        </div>
        <div>
          <label class="block text-xs font-semibold text-gray-500 mb-1">Nomi (Inglizcha)</label>
          <input name="name_en" value="<?php echo e(old('name_en', $stationeryCategory->name_en ?? '')); ?>" class="input">
        </div>
        <div>
          <label class="block text-xs font-semibold text-gray-500 mb-1">Nomi (Yaponcha)</label>
          <input name="name_ja" value="<?php echo e(old('name_ja', $stationeryCategory->name_ja ?? '')); ?>" class="input">
        </div>
        <div>
          <label class="block text-xs font-semibold text-gray-500 mb-1">Icon (emoji)</label>
          <input name="icon" value="<?php echo e(old('icon', $stationeryCategory->icon ?? '')); ?>" class="input" maxlength="10" placeholder="✏️">
        </div>
      ...
      <div class="flex items-center gap-3 mt-6 pt-5 border-t border-gray-100 dark:border-white/5">
        ...
        <a href="<?php echo e(route('admin.stationery-categories.index')); ?>" class="btn btn-secondary">Bekor qilish</a>
      </div>
    </form>
  </div>
</div>
...
<?php echo $__env->make('a122.layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/abbos/PROJECTS/MY/kitobchi-server/kitobchi-laravel/resources/views/a122/stationery-categories/edit.blade.php ENDPATH**/ ?>